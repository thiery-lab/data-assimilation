...
class EnsembleKalmanFilter(AbstractEnsembleFilter):
    ...
    def _assimilation_update(
        ...
        state_particles = (
            state_particles
            + (observation_errors @ nla.pinv(observation_deviations)) @ state_deviations
        )
...
class EnsembleSquareRootFilter(AbstractEnsembleFilter):
    ...
    def _assimilation_update(
        ...
        post_state_mean = state_mean + kalman_gain @ (observation - observation_mean)
        sqrt_transform_matrix = nla.cholesky(
            np.identity(num_particle)
...
class WoodburyEnsembleSquareRootFilter(AbstractEnsembleFilter):
    ...
    of observation noise covariance to avoid `O(dim_observation**3)` operations where
    `dim_observation` is the observation vector dimensionality.
    ...
    def _assimilation_update(
        ...
        sqrt_transform_matrix = nla.cholesky(
            np.identity(num_particle) - (c_matrix - c_matrix @ e_matrix) / num_particle
        )
        post_state_deviations = sqrt_transform_matrix.T @ state_deviations
        return (
            post_state_mean + post_state_deviations,
