...
def pairwise_euclidean_distance(z1, z2):
    ...
    return np.clip(z1_sq_norms - 2 * np.dot(z1, z2.T) + z2_sq_norms, 0, None)
...
def solve_optimal_transport_exact(
    ...
    trans_matrix, cost, results_code = _solve_ot_network_simplex(
        source_dist,
        target_dist,
        cost_matrix,
        max_iter=max_iter,
    ...
    if not results_code == 1:
        raise ConvergenceError(
            "OT solution did not converge. {0}: {1}".format(
                *get_result_code_strings(results_code)
            )
        )
    return trans_matrix
...
def solve_optimal_transport_exact_batch(
    ...
    num_problem, num_source, num_target = cost_matrices.shape
    # Check input arguments are dimensionally consistent
    assert (
        source_dists.shape[0] == num_problem and source_dists.shape[1] == num_source
    ), "source_dists shape incompatible with cost_matrices"
    assert (
        target_dists.shape[0] == num_problem and target_dists.shape[1] == num_target
    ), "target_dists shape incompatible with cost_matrices"
    results_codes = np.empty(num_problem, dtype="int32")
    costs = np.empty(num_problem, dtype="double")
    trans_matrices = np.empty(cost_matrices.shape, dtype="double", order="C")
    _solve_ot_network_simplex_batch(
        trans_matrices,
        costs,
        results_codes,
        source_dists,
    ...
    if not np.all(results_codes == 1):
        ...
        error_strings = [
            "{0}: {1}".format(*get_result_code_strings(e)) for e in err_codes
        ]
        num_error = np.sum(results_codes != 1)
        raise ConvergenceError(
            f"{num_error}/{num_problem} of OT solutions did not converge. "
            f"Errors: {', '.join(error_strings)}"
        )
...
def solve_optimal_transport_sinkhorn_batch(
    ...
    if max_marginal_error > 1e-8:
        warnings.warn(
            "Poor Sinkhorn--Knopp convergence. "
            "Max absolute marginal difference: ({0:.2e})".format(max_marginal_error)
        )
    return trans_matrices
...
if POT_AVAILABLE:
    ...
    def solve_optimal_transport_exact_pot(
        ...
        trans_matrix, log = ot.emd(
            ...
            np.ascontiguousarray(cost_matrix),
            numItermax=max_iter,
            log=True,
        )
        if not log["result_code"] == 1:
            warnings.warn(
                "OT solution did not converge. Error codes: {0}".format(
                    log["result_code"]
                )
            )
        return trans_matrix
    ...
    def solve_optimal_transport_sinkhorn_pot(
        source_dist, target_dist, cost_matrix, epsilon, max_iter=DEFAULT_MAX_ITER
    ):
    ...
    def solve_optimal_transport_exact_batch_pot(
        ...
        for p in range(n_problem):
            trans_matrices[p], log = ot.emd(
                ...
                np.ascontiguousarray(cost_matrices[p]),
                numItermax=max_iter,
                log=True,
            )
        ...
        if not np.all(result_codes == 1):
            ...
            warnings.warn(
                ...
                "Error codes: {2}".format(
                    np.sum(result_codes != 1), source_dists.shape[0], err_codes
                )
            )
        return trans_matrices

    def solve_optimal_transport_sinkhorn_batch_pot(
        source_dists, target_dists, cost_matrices, epsilon, max_iter=DEFAULT_MAX_ITER
    ):
        n_problem, n_particle = source_dists.shape
        trans_matrices = np.empty((n_problem, n_particle, n_particle))
        for p in range(n_problem):
